...
% TEST_FINALIZEBATCHVERDICTS  Unit tests for src/qa/finalizeBatchVerdicts.m
    ...
    methods (Static, Access = private)
        ...
        function g = pendingGate(label, stepIdx, metrics, sigmas, slack)
            ...
            % maxOutlierTrialPct = 1 opts the pctOutlierTrials field into the
    ...
    methods (Test)
        function single_gate_outlier_is_flagged(tc)
            % Five files with pctOutlierTrials = [1 2 3 4 50]. Outlier at 50.
            vals = [1 2 3 4 50];
            reports = cell(1, 5);
            for k = 1:5
                m = struct('pctOutlierTrials', vals(k), 'nFlatChans', NaN, ...
                    'nSatChans', NaN, 'pctOutlierChans', NaN, ...
        ...
        function absolute_mode_gates_untouched(tc)
            m = struct('pctOutlierTrials', 5, 'nFlatChans', NaN, ...
                'nSatChans', NaN, 'pctOutlierChans', NaN, ...
                'emgFraction', NaN, 'electrodeCount', NaN);
            absGate = struct( ...
        ...
        function WarnAt_overrides_slack_in_batch_mode(tc)
            ...
            extraM = struct('pctOutlierTrials', 7, 'nFlatChans', NaN, ...
                'nSatChans', NaN, 'pctOutlierChans', NaN, ...
                'emgFraction', NaN, 'electrodeCount', NaN);
            extraGate = struct( ...
                'label',      'g1', ...
                ...
                'thresholds', struct( ...
                    ...
                    'maxOutlierTrialPct', 1, 'maxOutlierTrialPctWarnAt', 4), ...
                'stepIndex',  5);
            ...
            for k = 1:numel(reports) - 1
                reports{k}.quality.gates{1}.thresholds.maxOutlierTrialPctWarnAt = 4;
            end
...
% Build N reports with one pending gate each carrying pctOutlierTrials=vals(k).
